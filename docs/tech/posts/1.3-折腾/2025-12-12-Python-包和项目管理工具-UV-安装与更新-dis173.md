...
<p align="center">
    <img src="https://kg.weiyan.cc/2025/12/uv-fast.webp" alt="uv-fast"><br>
...
![uv-install-base-url](https://kg.weiyan.cc/2025/07/uv-install-base-url.webp)    

![uv-install-curl-wget](https://kg.weiyan.cc/2025/07/uv-install-curl-wget.webp)

我们验证一下：
1. 首先，在 `install.sh` 的 `downloader` 部分增加一个 curl 执行输出。
   ![uv-install-echo-curl](https://kg.weiyan.cc/2025/07/uv-install-echo-curl.webp)

2. 其次，我们对下载后的结果解压缩（unpack the archive）的执行也增加一个输出。
   ![uv-install-tar](https://kg.weiyan.cc/2025/07/uv-install-tar.webp)

3. 最后，直接在本地执行 `sh install.sh`。
   ![uv-sh-install-sh](https://kg.weiyan.cc/2025/07/uv-sh-install-sh.webp)
